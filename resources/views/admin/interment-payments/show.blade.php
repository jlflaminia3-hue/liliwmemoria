            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-4">
                    <div>
                        ...
                        <div class="mt-2">
                            @php
                                $statusClass = match($deceased->payment_status) {
                                    'fully_paid' => 'success',
                                    'partial' => 'warning',
                                    default => 'danger'
                                };
                            @endphp
                            <span class="badge bg-{{ $statusClass }}">{{ $deceased->payment_status_label }}</span>
                        </div>
                    </div>
                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.interment-payments.index') }}" class="btn btn-light">
                                    ...
                                    <tbody>
                                        @foreach ($deceased->payments as $payment)
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <a class="btn btn-sm btn-light" href="{{ route('admin.interment-payments.invoice', [$deceased, $payment]) }}">Invoice</a>
                                                    @if ($payment->receipt_path)
                                                        <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.interment-payments.receipt', [$deceased, $payment]) }}">Receipt</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-borderless">
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-muted">Lot</td>
                                        <td>
                                            @if ($deceased->lot)
                                                {{ $deceased->lot->lot_number }} ({{ $deceased->lot->section }})
                                            @else
                                                -
                                            @endif
                                        </td>
                    ...
                    <div class="col-lg-4">
                        <div class="card border">
                            <div class="card-body">
                                ...
                                <form method="POST" action="{{ route('admin.interment-payments.store', $deceased) }}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" min="0" name="amount" class="form-control" value="{{ old('amount', $remainingBalance > 0 ? $remainingBalance : '') }}" placeholder="0.00" required>
                                        </div>
                                        @error('amount')
                                            <div class="text-danger small">{{ $message }}</div>
                                        @enderror
